...
class Processor:
    ...
    def store_bleu_values(self, line):
        ...
        if self.cur_src not in dict_to_use.keys():
            dict_to_use[self.cur_src] = dict()
        # we have only one src/tgt combination per dict and sw setting
    ...
    def main(self):
        ...
        directory_path = ""
        if "/" in bleu_file:
            split = bleu_file.rsplit("/",1)
            directory_path = split[0] + "/"
            bleu_file = split[1]
        output_path = directory_path + "average-bleu-scores-" + bleu_file
...
if __name__ == "__main__":
    # Debugging
    #proc = Processor("bleu-results.txt", False)
    #proc.main()
